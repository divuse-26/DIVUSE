...
if __name__ == '__main__':
    ...
    parser.add_argument('--part_src', nargs='*', help='Path to the part source file.',
                        default=None)
    parser.add_argument('--part_indices', type=str, help='Path to the part indices file.',
                        default=None)
    ...
    if not is_test:
        ...
        if args.processed_part_path is not None:
            ...
            if args.part_indices.endswith('.txt'):
                part_indices = []
                with open(args.part_indices) as file:
                    for line in file:
                        part_indices.append(int(line))
                example_dict = {example.id: example for example in part.train_examples}
                part.train_examples = [example_dict[i] for i in part_indices if i in example_dict]
            else:
                under = float(args.part_indices)
                if under != 1.0:
                    num = int(len(part.train_examples) * under)
                    random.shuffle(part.train_examples)
                    part.train_examples = part.train_examples[:num]
            debug(len(part.train_examples), len(part.valid_examples), len(part.test_examples),
                    len(part.test1_examples), len(part.test2_examples))
            dataset.train_examples += part.train_examples
            #############################################
            random.seed(args.sf)
            random.shuffle(dataset.train_examples)
            random.seed(args.seed)
            #############################################
            debug(len(dataset.train_examples), len(dataset.valid_examples), len(dataset.test_examples),
                  len(dataset.test1_examples), len(dataset.test2_examples))
    ...
    if is_test:
        ...
        model.train()
        model = train(model=model, dataset=dataset, max_steps=None, dev_every=200,
                      loss_function=loss_function, optimizer=optim,
                      save_path=model_dir + '/GGNNSumModel' + str(args.sf), max_patience=20, log_every=None,
